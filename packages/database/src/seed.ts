...
async function main(){
  ...
  for(const seed of regionSeeds){
    ...
    const outreachBrandName=seed.key==="ottawa"?"Dexa":"Saturn Star Movers";await prisma.regionLaunchConfiguration.upsert({where:{regionId:region.id},update:{marketplaceDiscoveryEnabled:true,outreachEnabled:true,followUpsEnabled:true,crmPushEnabled:true,dailyTaskLimit:50,dailySellerContactLimit:50,minimumOpportunityScore:70,outreachBrandName},create:{regionId:region.id,marketplaceDiscoveryEnabled:true,outreachEnabled:true,followUpsEnabled:true,crmPushEnabled:true,dailyTaskLimit:50,dailySellerContactLimit:50,minimumOpportunityScore:70,outreachBrandName}});

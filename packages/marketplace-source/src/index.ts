...
export function parseFacebookSearchText(text:string):FacebookMcpListing[]{
  ...
  for(const block of blocks){const heading=block.match(/^\s*\d+\.\s+\*\*(.+?)\*\*\s+—\s+(.+)$/m),url=block.match(/https:\/\/www\.facebook\.com\/marketplace\/item\/(\d+)\/?/);if(!heading||!url)continue;const meta=block.match(/📍\s*(.*?)\s*\|\s*👤\s*(.*?)(?:\s*⏳\s*PENDING)?\s*$/m),imageUrl=block.match(/🖼️\s*(https?:\/\/\S+)/)?.[1];listings.push({id:url[1]!,title:heading[1]!,price:heading[2]!.trim(),location:meta?.[1]?.trim(),sellerName:meta?.[2]?.trim(),imageUrl,url:url[0],isPending:/PENDING/.test(block)});}return listings;
}

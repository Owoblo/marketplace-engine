...
export function evaluateInitialOutreach(input:OutreachPolicyInput){
  ...
  const elapsed=input.sellerLastContactAt?((input.now??new Date()).getTime()-input.sellerLastContactAt.getTime())/86400000:Infinity;if(elapsed<(input.cooldownDays??30)&&!input.manualOverride)return {allowed:false,reason:"seller_cooldown"} as const;return {allowed:true,reason:"eligible"} as const;
}
